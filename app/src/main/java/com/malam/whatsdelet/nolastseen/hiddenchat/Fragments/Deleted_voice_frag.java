...
public class Deleted_voice_frag extends Fragment {
View root_view;
Context context;
...
public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
    ...
    if (root_view == null) {
        ...
        muList.clear();
        new bg_task().execute();
        LocalBroadcastManager.getInstance(context).registerReceiver(refresh_voice, new IntentFilter(context.getResources().getString(R.string.intent_file_saved_voice)));
        
    }
    return root_view;
}

private BroadcastReceiver refresh_voice = new BroadcastReceiver() {
...
private BroadcastReceiver delete_voice = new BroadcastReceiver() {
            ...
            public void run() {
                try {
                    if (muList.size() > 0) {
                        ...
                        no_file_found_title.setText(getResources().getString(R.string.voice_head));
                        no_file_found.setText(getResources().getString(R.string.no_voice_msg));
                    }
...
public class My_Adapter extends RecyclerView.Adapter<My_Adapter.MyHolder> {
    ...
    public void onBindViewHolder(final MyHolder holder, final int position) {
        try {
            ...
            if (!Shared.getInstance().getBooleanValueFromPreference(context.getResources().getString(R.string.long_pressd_deleted_voice_pref), false, context)) {
            ...
            holder.delete_music.setImageResource(R.mipmap.play);
            holder.delete_music.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
            ...
            holder.main_layout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (Shared.getInstance().getBooleanValueFromPreference(context.getResources().getString(R.string.long_pressd_deleted_voice_pref), false, context)) {
                        ...
                        File file = new File(muList.get(getRealPosition(position)).getFile_path());
                        Intent intent = new Intent();
                        Uri voice_uri = FileProvider.getUriForFile(context, context.getApplicationContext().getPackageName() + ".provider", file);
                        intent.setAction(Intent.ACTION_VIEW);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                        intent.addFlags(Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
                        intent.setDataAndType(voice_uri, "audio/*");
                        mycontxt.startActivity(Intent.createChooser(intent, getResources().getString(R.string.play_sound_via)));
                    }*/
                }
            });
            holder.main_layout.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if (!Shared.getInstance().getBooleanValueFromPreference(context.getResources().getString(R.string.long_pressd_deleted_voice_pref), false, context)) {
                        Shared.getInstance().saveBooleanToPreferences(context.getResources().getString(R.string.long_pressd_deleted_voice_pref), true, context);
                        MainActivity.selected_items++;
                        muList.get(getRealPosition(position)).setIs_selected(true);
                        MainActivity.check_selected_items();
                        notifyDataSetChanged();
                    ...
                    Shared.getInstance().saveIntToPreferences(getResources().getString(R.string.delete_tabs_position_pref), 7, context);
                    Shared.getInstance().saveStringToPreferences(getResources().getString(R.string.delete_diloug_title_pref), getResources().getString(R.string.delete_voice_title), context);
                    Shared.getInstance().saveStringToPreferences(getResources().getString(R.string.delete_dialoug_msg_pref), getResources().getString(R.string.delete_voice_msg), context);
...
*/
    ...
    class MyHolder extends RecyclerView.ViewHolder {
        ImageView imgV, selectedImage, delete_music, share_music;
        ...
        public MyHolder(View itemView) {
            ...
            delete_music = itemView.findViewById(R.id.play_btn);
