...
public abstract class FixedFileObserverVideo {
    private final static HashMap<File, Set<FixedFileObserverVideo>> sObserverLists = new HashMap<>();

    private FileObserver mObserver;
    private final File mRootPath;
    private final int mMask;

    public FixedFileObserverVideo(String path) {
        this(path, FileObserver.ALL_EVENTS);
    }

    public FixedFileObserverVideo(String path, int mask) {
    ...
    public void startWatching() {
        synchronized (sObserverLists) {
            if (!sObserverLists.containsKey(mRootPath))
                sObserverLists.put(mRootPath, new HashSet<FixedFileObserverVideo>());

            final Set<FixedFileObserverVideo> fixedObservers = sObserverLists.get(mRootPath);
                ...
                public void onEvent(int event, String path) {
                    switch (event) {
                        ...
                        case FileObserver.DELETE:
                            for (FixedFileObserverVideo fixedObserver : fixedObservers)
                        ...
                        case FileObserver.MOVED_TO:
                            for (FixedFileObserverVideo fixedObserver : fixedObservers)
                                if ((event & fixedObserver.mMask) != 0)
                                    fixedObserver.onEvent(event, path);
//                            FilesData.getDeletedMediaVideio(path);
                            break;
    ...
    public void stopWatching() {
        synchronized (sObserverLists) {
            Set<FixedFileObserverVideo> fixedObservers = sObserverLists.get(mRootPath);
