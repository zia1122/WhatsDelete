...
public class Chat_contact_Adapter extends RecyclerView.Adapter<Chat_contact_Adapter.MyViewHolder> {

private Context context;
boolean type;
private List<Note_new> notesList;
public static List<Note_new> selected_msgs = new ArrayList<>();

private static final int CONTENT_TYPE = 0;

private static final int AD_TYPE = 1;
private int LIST_AD_DELTA = 0;

public UnifiedNativeAdView adView;
public static UnifiedNativeAd my_unifiedNativeAd = null;
Boolean ad_request_sent = false;
int my_position = 0;
Boolean on_ad_loaded = false, long_pressed = false;

public void notifystatus() {
    this.notesList.get(notesList.size() - 1).setstatus(true);
...
public static class MyViewHolder extends RecyclerView.ViewHolder {
    public TextView contact_chat_textview_sender, contact_chat_textview_reciever, call_time_txtview, date_n_time;
    public TextView dot;
    public TextView timestamp, call_time_txtview_reciver;
    RelativeLayout main_layout;
    ImageView selected;
    LinearLayout sender_layout, reciver_layout;
    TextView deleted_tag;
    ...
    public MyViewHolder(View view) {
        ...
        contact_chat_textview_reciever = view.findViewById(R.id.contact_chat_textview_reciever);
        
        call_time_txtview = view.findViewById(R.id.call_time_txtview_reciver);
        date_n_time = view.findViewById(R.id.date_n_time);
        deleted_tag = view.findViewById(R.id.deltag);
        dot = view.findViewById(R.id.dot);
        timestamp = view.findViewById(R.id.timestamp);
        call_time_txtview_reciver = view.findViewById(R.id.call_time_txtview);
...
public Chat_contact_Adapter(Boolean type, Context context, List<Note_new> notesList, int size) {
...
public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        /*switch (viewType) {
            ...
            case CONTENT_TYPE:
                View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.chat_contact_row, parent, false);
                return new MyViewHolder(itemView);  // Fall through.
            default:
                break;
        }*/
    return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.chat_contact_row, parent, false));
...
public void onBindViewHolder(final MyViewHolder holder, int position) {
    ...
    try {
        if (notesList.get(getRealPosition(position)).getPath().equals(context.getResources().getString(R.string.from_sender))) {
            holder.sender_layout.setVisibility(View.VISIBLE);
            holder.reciver_layout.setVisibility(View.GONE);
            holder.contact_chat_textview_reciever.setText(notesList.get(getRealPosition(position)).getNumber());
            holder.deleted_tag.setVisibility(View.GONE);
        } else {
            holder.sender_layout.setVisibility(View.GONE);
            holder.reciver_layout.setVisibility(View.VISIBLE);
            String msg = notesList.get(getRealPosition(position)).getNumber();
            String title = notesList.get(getRealPosition(position)).getNumber();
            Boolean gourp = false;
            Boolean status = notesList.get(getRealPosition(position)).getstatus();
           // holder.contact_chat_textview_reciever.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);
            if (status) {
                holder.deleted_tag.setVisibility(View.VISIBLE);
                holder.contact_chat_textview_sender.setTextColor(context.getResources().getColor(R.color.red));
                //holder.contact_chat_textview_sender.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);
                
            } else {
                holder.deleted_tag.setVisibility(View.GONE);
                holder.contact_chat_textview_sender.setTextColor(Color.BLACK);
               // holder.contact_chat_textview_sender.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);
            }
            
//            holder.contact_chat_textview_sender.setMovementMethod(LinkMovementMethod.getInstance());
//            holder.call_time_txtview_reciver.setMovementMethod(LinkMovementMethod.getInstance());
            try {
                if (msg.contains(":")) {
                    gourp = true;
            ...
            if (gourp) {
                ...
                holder.contact_chat_textview_sender.setText(Html.fromHtml(sourceString));
            } else {
                holder.contact_chat_textview_sender.setText(msg);
            }
            //holder.note.setText(notesList.get(getRealPosition(position)).getNumber());
        }
        ...
        holder.call_time_txtview.setText(notesList.get(getRealPosition(position)).getTime());
        holder.call_time_txtview_reciver.setText(notesList.get(getRealPosition(position)).getTime());
        holder.timestamp.setText(notesList.get(getRealPosition(position)).getTimestamp());
            ...
            public void onClick(View v) {
                ...
                if (Shared.getInstance().getBooleanValueFromPreference(context.getResources().getString(R.string.long_pressd_specific_chat_pref), false, context)) {
                    ...
                    if (selected_msgs.size() < 1) {
                        if (type)
                            Contact_Chat_Act.share.setVisibility(View.GONE);
                        else Contact_Chat_Act_fro_Notification.share.setVisibility(View.GONE);
                        ...
                        if (selected_msgs.size() < 2) {
                            if (type) Contact_Chat_Act.share.setVisibility(View.VISIBLE);
                            else Contact_Chat_Act_fro_Notification.share.setVisibility(View.VISIBLE);
                        }
                        else  if (type)
                            Contact_Chat_Act.share.setVisibility(View.GONE);
                        else Contact_Chat_Act_fro_Notification.share.setVisibility(View.GONE);
